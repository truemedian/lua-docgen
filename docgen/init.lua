...
do
	...
	patterns.type_annotated =
					Ct(Cg(type_name, 'name') * Cg(type_anchor ^ -1, 'anchor') * Cg(type_annotation, 'annotation'))
	...
	for _, field in ipairs(fields) do
		...
		elseif field.name == 'param?' then
			...
			params.list[#params.list + 1] = {
				name = name,
				types = types,
				default = trim(param_type:sub(stop + 1)),
				optional = true,
			}
			params.simple[#params.simple + 1] = name
		end
...
local function parse_docstring_chunk(file_state, chunk)
	...
	elseif get_field(fields, 'constructor') then
		local class = assert(file_state.class,
		                     'missing class documentation, attempted to parse constructor in: ' .. file_state.src)
		local constructor = {}
	...
	elseif get_field(fields, 'metamethod') then
		local class = assert(file_state.class,
		                     'missing class documentation, attempted to parse metamethod in: ' .. file_state.src)
		local metamethod = {}
	...
	elseif get_field(fields, 'method') then
		local class =
						assert(file_state.class, 'missing class documentation, attempted to parse method in: ' .. file_state.src)
		local method = {}
	...
	elseif get_field(fields, 'static') then
		local class = assert(file_state.class,
		                     'missing class documentation, attempted to parse static method in: ' .. file_state.src)
		local static = {}
	...
	elseif get_field(fields, 'property') then
		local class = assert(file_state.class,
		                     'missing class documentation, attempted to parse property in: ' .. file_state.src)
		local property = {}
...
local function emit_class(buffer, classes, class)
	...
	if class.constructor then
		...
		write(buffer, '<a class="anchor" href="#constructor">', class.name, '(',
		      table.concat(class.constructor.params.simple, ', '), ')</a>', '\n')
	...
	for _, methods in ipairs(class.statics) do
		...
		for _, method in ipairs(methods.list) do
			...
			write(buffer, '<a class="anchor" href="#static-', slugify(method.name), '">', method.name, '(',
			      table.concat(method.params.simple, ', '), ')</a>', '\n')
	...
	for _, methods in ipairs(class.methods) do
		...
		for _, method in ipairs(methods.list) do
			...
			write(buffer, '<a class="anchor" href="#method-', slugify(method.name), '">', method.name, '(',
			      table.concat(method.params.simple, ', '), ')</a>', '\n')
...
local docgen = {
	new_state = new_state,
	parse_file_chunk = parse_file_chunk,
	parse_enumerations = parse_enumerations,
	parse_directory = parse_directory,
}

local file = process.argv[1]
local parts = file and path.splitPath(file) or {}
local first = table.remove(parts)

if first == 'docgen' then
	print('Discordia Documentation Generator')

	-- luvit docgen.lua dest
	-- luvit docgen.lua libs_src dest

	local source = (process.argv[3] and process.argv[2]) or './libs'
	local output = (process.argv[3] or process.argv[2]) or './docs'
	...
	write_documentation(state, source, output)
end
