...
local lua_keywords = 'break do else elseif end for function if in local repeat return then until while'
...
function lxsh.highlighters.new(context)
  ...
  local function decorator(subject)
    ...
    for kind, text in context.lexer.gmatch(subject, { join_identifiers = false }) do
      ...
      elseif kind == 'comment' or kind == 'constant' or kind == 'string' then
        ...
        if kind == 'identifier' and lua_keywords:find(text, 1, true) then
          coroutine.yield('keyword', text)
        else
  ...
  return function(subject, options)
    local output = {}
    local options = typ(options) == 'table' and options or {}
